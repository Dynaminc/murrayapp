...
def sign_in(request):
    ...
    if serializer.is_valid():
        ...
        if authenticated_user:
            ...
            profile_instance = Profile.objects.filter(user=authenticated_user).first()
            profile_instance.update_token()
            return JsonResponse({'message': 'Welcome back! You are now logged in.',
                                    'response': {                     
                                        'jwt_token': str(profile_instance.token),
                                        'refresh_token': str(profile_instance.token)
                                        }, 
                                    'status': 200},

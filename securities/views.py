...
def test_end(request):
    ...
    try:
        ...
        if latest_data:
            ...
            combs = [{'symbol':item.symbol,'stdev':item.stdev,'score':item.z_score,'date':str(datetime.now())} for item in Combination.objects.all() if str(item.date_time).split('.')[0].split(':')[:-1] ==  latest_time]
